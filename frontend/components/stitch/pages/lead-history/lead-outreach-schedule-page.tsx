...
export function LeadOutreachSchedulePage() {
  ...
  const tenantMonitorQuery = useTenantOutreachMonitor(
    Boolean(currentUserQuery.data?.tenantId)
  )
  ...
  const retryJobMutation = useRetryTenantOutreachJob()
  ...
  const filteredLeadOptions = useMemo(() => {
    ...
    return leadOptions.filter((lead) =>
      [lead.name, lead.email, lead.phone, lead.property].some((value) =>
  ...
  const dealStagePreview = useMemo(() => {
    ...
    items.forEach((item) => {
      ...
      leadMap.set(
        item.sourceLeadId,
        displayText(item.sourceLeadName, `Lead #${item.sourceLeadId}`)
      )
    })
    ...
    dealStagePreviewQuery.data?.items,
    dealStagePreviewQuery.data?.totalCount,
  ])
  ...
  const paginatedSchedule = useMemo(
    ...
    [filteredSchedule, page]
  )
  const actionCounts = useMemo(
    () => ({
      cancelled: filteredSchedule.filter((entry) => entry.status === "Failed")
        .length,
      completed: filteredSchedule.filter(
        (entry) => entry.status === "Completed" || entry.status === "Sent"
      ).length,
      scheduled: filteredSchedule.filter(
  ...
  async function handleSaveSchedule() {
    ...
    const titleForKind = (kind: "Email" | "Sms") =>
      kind === "Email"
        ? composer.title.trim()
        : composer.title.trim() || "SMS follow-up"
    ...
    if (composer.audienceType === "SingleLead") {
      ...
      if (failed) {
        setSubmitError(
          failed.error?.message || "Unable to save the schedule item."
        )
        return
      }
    ...
    const responses = await Promise.all(
      selectedKinds.map((kind) =>
        bulkDispatchMutation.mutateAsync({
          ...
          leadStage:
            composer.audienceType === "LeadStage"
              ? composer.leadStage || null
              : null,
          dealStage:
            composer.audienceType === "DealStage"
              ? composer.dealStage || null
              : null,
          kind,
          title: titleForKind(kind),
          message: composer.message.trim(),
    ...
    if (failed) {
      setSubmitError(
        failed.error?.message || "Unable to save the bulk schedule items."
      )
      return
    }
    ...
    if (failures.length > detailLines.length) {
      detailLines.push(
        `+${failures.length - detailLines.length} more failure(s)`
      )
    }
    ...
    <main className="min-h-full bg-[var(--ether-surface)] p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1600px] space-y-7">
        <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="ether-display-lg text-[var(--ether-on-surface)]">
              Lead Outreach Management
            </h1>
            <p className="mt-2 text-base text-[var(--ether-on-surface-variant)]">
              Track and coordinate high-velocity lead outreach from a unified
              command center.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Button
              className="h-11 rounded-lg border-[var(--ether-primary)] bg-[var(--ether-primary-fixed)] px-5 font-semibold text-[var(--ether-primary)]"
              render={<Link href="/dashboard/mail" />}
              variant="outline"
            >
              Open Mail
            </Button>
        ...
        <section className="grid gap-6 md:grid-cols-3">
          <ActionMetric
            icon="schedule"
            label="Scheduled now"
            value={actionCounts.scheduled}
            tone="primary"
          />
          <ActionMetric
            icon="check_circle"
            label="Sent / completed"
            value={actionCounts.completed}
            tone="secondary"
          />
          <ActionMetric
            icon="error"
            label="Paused / failed"
            value={actionCounts.cancelled}
            tone="tertiary"
          />
        </section>

        {tenantMonitorQuery.data ? (
          <section className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="ether-label-caps text-[var(--ether-primary)]">
                  Tenant delivery queue
                </p>
                <h2 className="mt-2 text-xl font-bold text-[var(--ether-on-surface)]">
                  Isolated worker health
                </h2>
              </div>
              <p className="text-xs text-[var(--ether-outline)]">
                {tenantMonitorQuery.data.outstanding} outstanding ·{" "}
                {tenantMonitorQuery.data.failedAttemptsLast24Hours} failed
                attempts in 24h
              </p>
            </div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              {(
                ...
                <QueueMetric
                  key={status}
                  status={status}
                  value={tenantMonitorQuery.data.statuses[status]}
                />
              ))}
            </div>
        ...
        <section className="overflow-hidden rounded-[24px] bg-white shadow-[var(--shadow-surface-1)]">
          <div className="flex flex-col gap-3 p-5 xl:flex-row xl:items-center">
            ...
            >
              <SelectTrigger className="h-12 w-full rounded-xl border-[var(--ether-outline-variant)] bg-white px-4 xl:w-44">
                <span>
                  {stageFilter ? formatLeadStage(stageFilter) : "Lead Stage"}
                </span>
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
            ...
            >
              <SelectTrigger className="h-12 w-full rounded-xl border-[var(--ether-outline-variant)] bg-white px-4 xl:w-40">
                <span>
                  {kindFilter ? formatKindLabel(kindFilter) : "Channel"}
                </span>
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setStatusFilter(
                  value === "all" ? "" : (value as LeadHistoryStatus)
                )
              }
            ...
            >
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  {["Scheduled", "Sent", "Completed", "Received", "Failed"].map(
                    (status) => (
            ...
            <Select
              ...
              onValueChange={(value) =>
                setFollowUpFilter(
                  value === "all" ? "" : (value as FollowUpFilter)
                )
              }
          ...
          <div className="overflow-x-auto">
            {scheduleQuery.isLoading ? (
              <div className="p-10 text-center text-sm font-semibold text-[var(--ether-outline)]">
                Loading schedule...
              </div>
            ) : scheduleQuery.error ? (
              <div className="m-6 rounded-xl bg-[var(--ether-error-container)] p-4 text-sm font-semibold text-[var(--ether-error)]">
                {scheduleQuery.error.message}
              </div>
            ) : filteredSchedule.length === 0 ? (
              <div className="p-10 text-center text-sm font-semibold text-[var(--ether-outline)]">
                No lead activity matches the current filters.
              </div>
            ) : (
              <Table className="min-w-[1180px]">
                ...
                <TableBody>
                  {paginatedSchedule.map((entry, index) => {
                    ...
                    return (
                      <TableRow
                        className="border-0 transition hover:bg-[var(--ether-surface-container-low)]"
                      ...
                      >
                        <TableCell className="px-6 py-5">
                          <div className="flex items-center gap-3">
                              ...
                              className={cn(
                                ...
                                index % 3 === 0
                                  ...
                                  : index % 3 === 1
                                    ...
                                    : "bg-[var(--ether-tertiary-fixed)] text-[var(--ether-tertiary)]"
                              )}
                            >
                              {initials || "LD"}
                            </span>
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 max-w-64 truncate text-xs text-[var(--ether-on-surface-variant)]">
                                {displayText(
                                  entry.leadProperty,
                                  "No property linked"
                                )}{" "}
                                ?{" "}
                                {displayText(
                                  entry.leadEmail || entry.leadPhone,
                                  "No contact"
                                )}
                              </p>
                              <div className="mt-2 flex flex-wrap gap-1">
                                {entry.leadStage ? (
                                  <Badge className="rounded-md border-0 bg-[var(--ether-surface-container-high)] px-2 py-1 text-[9px] font-bold text-[var(--ether-on-surface-variant)] uppercase">
                                    {formatLeadStage(
                                      entry.leadStage as LeadStage
                                    )}
                                  </Badge>
                                ) : null}
                                {entry.leadPriority ? (
                                  <Badge className="rounded-md border-0 bg-[var(--ether-primary-fixed)] px-2 py-1 text-[9px] font-bold text-[var(--ether-primary)] uppercase">
                                    {entry.leadPriority}
                                  </Badge>
                                ) : null}
                              </div>
                        ...
                        <TableCell>
                          <Badge className="rounded-full border-0 bg-[var(--ether-surface-container-high)] px-3 py-1.5 text-xs font-semibold text-[var(--ether-on-surface-variant)]">
                        ...
                        <TableCell className="pr-6">
                          <div className="flex flex-wrap justify-end gap-2">
                            <Button
                              className="h-9 rounded-lg px-3 text-xs font-semibold text-[var(--ether-on-surface-variant)] hover:bg-[var(--ether-primary-fixed)] hover:text-[var(--ether-primary)]"
                              render={
                                <Link
                            ...
                            <Button
                              ...
                              variant="ghost"
                            >
                              <AppIcon name="visibility" /> Details
                            </Button>
                            <Button
                              className="h-9 rounded-lg px-3 text-xs font-semibold text-[var(--ether-on-surface-variant)] hover:bg-amber-100 hover:text-amber-700"
                              disabled={
                                entry.status !== "Scheduled" ||
                                scheduleStatusMutation.isPending
                              }
                              onClick={() =>
                                void scheduleStatusMutation.mutateAsync({
                                  id: entry.id,
                                  status: "paused",
                                })
                              }
                              ...
                              variant="ghost"
                            >
                              <AppIcon name="pause_circle" /> Pause
                            </Button>
                            <Button
                              className="h-9 rounded-lg px-3 text-xs font-semibold text-[var(--ether-on-surface-variant)] hover:bg-[color-mix(in_srgb,var(--ether-secondary-container)_30%,white)] hover:text-[var(--ether-secondary)]"
                              disabled={
                                entry.status === "Scheduled" ||
                                !entry.scheduledAt ||
                                scheduleStatusMutation.isPending
                              ...
                              variant="ghost"
                            >
                              <AppIcon name="play_circle" /> Resume
                            </Button>
                            {entry.queueStatus === "failed" ||
                            entry.queueStatus === "dead_letter" ||
                            entry.queueStatus === "cancelled" ? (
                              <Button
                                className="h-9 rounded-lg px-3 text-xs font-semibold text-[var(--ether-error)] hover:bg-[var(--ether-error-container)]"
                                disabled={retryJobMutation.isPending}
                                onClick={() =>
                                  void retryJobMutation.mutateAsync({
                                    id: entry.id,
                                  })
                                }
                                type="button"
          ...
          {filteredSchedule.length > 0 ? (
            <div className="flex flex-col gap-3 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-[var(--ether-on-surface-variant)]">
                ...
                {Math.min(page * pageSize, filteredSchedule.length)} of{" "}
        ...
        <Sheet open={composerOpen} onOpenChange={setComposerOpen}>
          <SheetContent
            className="!w-full overflow-hidden border-0 bg-[var(--ether-surface)] p-0 shadow-[-20px_0_60px_rgba(11,28,48,0.16)] sm:!max-w-none lg:!w-[44vw] lg:!max-w-[44vw]"
            side="right"
          >
            <SheetHeader className="bg-white px-6 pt-6 pb-5 text-left">
              <SheetTitle className="text-2xl font-bold tracking-[-0.02em]">
                New Outreach
              </SheetTitle>
              <SheetDescription>
                Send now or schedule email plus SMS/MMS from one place.
              </SheetDescription>
            </SheetHeader>
            <div className="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6">
              <section className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
                <h3 className="ether-headline-sm">Audience</h3>
                <div className="mt-4 grid gap-2 sm:grid-cols-3">
                  {[
                    {
                      icon: "person",
                      label: "Single Lead",
                      value: "SingleLead" as const,
                    },
                    {
                      icon: "group",
                      label: "Lead Stage",
                      value: "LeadStage" as const,
                    },
                    {
                      icon: "contract",
                      label: "Deal Stage",
                      value: "DealStage" as const,
                    },
                  ].map((option) => (
                    <Button
                      className={cn(
                        "rounded-lg",
                        composer.audienceType === option.value
                          ? "bg-[var(--ether-primary)] text-white"
                          : "bg-[var(--ether-surface-container-low)] text-[var(--ether-on-surface-variant)]"
                      )}
                      key={option.value}
                      onClick={() =>
                        updateComposer({ audienceType: option.value })
                      }
                      type="button"
                      variant="ghost"
                    >
              ...
              <section className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
                {composer.audienceType === "SingleLead" ? (
                  <div className="space-y-3">
                    ...
                    <Input
                      className="h-11 rounded-lg border-[var(--ether-outline-variant)]"
                    ...
                    >
                      <SelectTrigger className="h-11 rounded-lg border-[var(--ether-outline-variant)]">
                        <span>
                          {selectedLead
                            ? displayText(
                                selectedLead.name,
                      ...
                      <SelectContent>
                        ...
                        {filteredLeadOptions.map((item) => (
                          ...
                          >{`${displayText(item.name, `Lead #${item.id}`)} - ${displayText(item.phone || item.email, "No contact")}`}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {composer.audienceType === "LeadStage" ? (
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateComposer({
                        leadStage: value === "none" ? "" : (value as LeadStage),
                      })
                    }
                  ...
                  >
                    <SelectTrigger className="h-11 rounded-lg border-[var(--ether-outline-variant)]">
                      <span>
                        {composer.leadStage
                          ? formatLeadStage(composer.leadStage)
                          : "Choose a lead stage"}
                      </span>
                    </SelectTrigger>
                    <SelectContent>
                ...
                {composer.audienceType === "DealStage" ? (
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateComposer({
                        dealStage: value === "none" ? "" : (value as DealStage),
                      })
                    }
              ...
              <section className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="flex items-center gap-3 rounded-lg bg-[var(--ether-surface-container-low)] p-3 text-sm font-semibold">
                    <Checkbox
                      checked={composer.sendEmail}
                      onCheckedChange={(checked) =>
                        updateComposer({ sendEmail: checked === true })
                      }
                    />
                    Email
                  </label>
                  <label className="flex items-center gap-3 rounded-lg bg-[var(--ether-surface-container-low)] p-3 text-sm font-semibold">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        updateComposer({ sendSms: checked === true })
                      }
                    />
                    SMS / MMS
                  </label>
                ...
                <div className="mt-4 grid gap-4">
                  <Input
                    className="h-11 rounded-lg border-[var(--ether-outline-variant)]"
                    onChange={(event) =>
                      updateComposer({ scheduledAt: event.target.value })
                    }
                    type="datetime-local"
                    ...
                    onValueChange={(value) => {
                      ...
                      updateComposer({
                        ...
                        title: template
                          ...
                          : composer.title,
                        message: template
                          ? resolveTemplateTokens(template.body, selectedLead)
                          : composer.message,
                      })
                    }}
                      ...
                      <span>
                        {composer.templateId
                          ? (availableTemplates.find(
                              (item) => item.id === composer.templateId
                            )?.name ?? "Template")
                          : "No template"}
                      </span>
                    </SelectTrigger>
                  ...
                  {composer.sendSms ? (
                    <Textarea
                      className="min-h-20 rounded-lg border-[var(--ether-outline-variant)]"
                      onChange={(event) =>
                        updateComposer({ mediaUrls: event.target.value })
                      }
                      placeholder="MMS attachment URLs"
                      value={composer.mediaUrls}
                    />
                  ) : null}
                  <Textarea
                    className="min-h-40 rounded-lg border-[var(--ether-outline-variant)]"
                    onChange={(event) =>
                      updateComposer({ message: event.target.value })
                    }
              ...
              {submitError ? (
                <div className="rounded-xl bg-[var(--ether-error-container)] p-4 text-sm font-semibold text-[var(--ether-error)]">
                  {submitError}
                </div>
              ) : null}
              {submitFeedback ? (
            ...
            <SheetFooter className="bg-white px-6 py-4 shadow-[0_-10px_30px_rgba(11,28,48,0.06)]">
              <Button
                className="h-11 w-full rounded-lg bg-[var(--ether-primary)] font-semibold text-white"
                disabled={isSaving}
        ...
        >
          <SheetContent className="w-full border-0 bg-[var(--ether-surface)] p-0 sm:max-w-[40rem]">
            <SheetHeader className="bg-white px-6 py-6 text-left">
              ...
              <SheetDescription>
                {selectedDetail
                  ? `${displayText(selectedDetail.leadName, `Lead #${selectedDetail.leadId}`)} ? ${formatKindLabel(selectedDetail.kind)}`
                  : ""}
              </SheetDescription>
            </SheetHeader>
            {selectedDetail ? (
              <div className="custom-scrollbar flex-1 space-y-5 overflow-y-auto p-6">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <DetailPill
                    label="Stage"
                    value={
                      selectedDetail.leadStage
                        ? formatLeadStage(selectedDetail.leadStage as LeadStage)
                        : "No stage"
                    }
                  />
                  <DetailPill
                    label="Channel"
                    value={formatKindLabel(selectedDetail.kind)}
                  />
                  <DetailPill
                    label="Type"
                ...
                <div className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
                  ...
                  <p className="mt-3 font-bold">
                    {displayText(
                      ...
                      `Lead #${selectedDetail.leadId}`
                    )}
                  </p>
                  <p className="mt-1 text-sm text-[var(--ether-on-surface-variant)]">
                    {displayText(selectedDetail.leadEmail)}
                  </p>
                  <p className="text-sm text-[var(--ether-on-surface-variant)]">
                    {displayText(selectedDetail.leadPhone)}
                  </p>
                  <p className="mt-3 text-sm font-semibold">
                    {displayText(
                      selectedDetail.leadProperty,
                      "No property linked"
                    )}
                  </p>
                </div>
                <div className="rounded-[24px] bg-white p-5 shadow-[var(--shadow-surface-1)]">
                  ...
                  <p className="mt-3 font-bold">
                    {displayText(selectedDetail.title, "Untitled")}
                  </p>
                  <p className="mt-2 text-sm text-[var(--ether-on-surface-variant)]">
                    {displayText(selectedDetail.summary, "No summary")}
                  </p>
                  <div className="mt-3 rounded-xl bg-[var(--ether-surface-container-low)] p-4 text-sm leading-6 whitespace-pre-wrap">
                    {displayText(selectedDetail.body, "No message body saved.")}
                  </div>
                </div>
                <div className="rounded-[24px] bg-white p-5 text-sm shadow-[var(--shadow-surface-1)]">
                  <p className="ether-label-caps text-[var(--ether-outline)]">
                    Timing
                  </p>
                  {selectedDetail.scheduledAt ? (
                    <p className="mt-3">
                      Scheduled:{" "}
                      {formatDateTimeLabel(selectedDetail.scheduledAt)}
                    </p>
                  ) : null}
                  {selectedDetail.occurredAt ? (
                    <p className="mt-2">
                      Occurred: {formatDateTimeLabel(selectedDetail.occurredAt)}
                    </p>
                  ) : null}
                  <p className="mt-2">
                    Saved: {formatDateTimeLabel(selectedDetail.createdAt)}
                  </p>
                </div>
              </div>
            ) : null}
            <SheetFooter className="bg-white px-6 py-4">
              <Button
                className="w-full"
                render={
                  selectedDetail ? (
                    <Link
                      href={buildHistoryHref(pathname, selectedDetail.leadId)}
                    />
                  ) : (
                    <span />
                  )
    ...
    <article className="rounded-[24px] bg-white p-6 shadow-[var(--shadow-surface-1)]">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p
            className={cn(
              "ether-numeric-lg mt-2",
              tone === "secondary"
                ? "text-[var(--ether-secondary)]"
                : tone === "tertiary"
        ...
        {bars.map((height, index) => (
          <span
            className={cn(
              "flex-1 rounded-t",
              tone === "secondary"
                ? "bg-[var(--ether-secondary)]/25"
                : tone === "tertiary"
                  ? "bg-[var(--ether-error)]/18"
                  : "bg-[var(--ether-primary)]/18",
              index === 5 &&
                (tone === "secondary"
                  ? "bg-[var(--ether-secondary)]"
                  : tone === "tertiary"
                    ? "bg-[var(--ether-error)]"
...
function QueueMetric({
  ...
  const warning = status === "retrying" || status === "processing"
  return (
    <div
      className={cn(
        "rounded-xl border p-4",
        danger
          ? "border-[var(--ether-error)]/20 bg-[var(--ether-error-container)]"
          : warning
            ? "border-amber-300 bg-amber-50"
            : "border-[var(--ether-outline-variant)] bg-[var(--ether-surface-container-low)]"
      )}
    >
      <p className="text-[10px] font-bold tracking-[0.08em] text-[var(--ether-on-surface-variant)] uppercase">
        {labels[status]}
      </p>
      <p className="mt-2 text-2xl font-black text-[var(--ether-on-surface)]">
        {value.toLocaleString("en-US")}
      </p>
    </div>
  )
...
function SyncMetric({
  ...
  const danger = status === "failed" || status === "dead_letter"
  const warning = status === "processing" || status === "retrying"
  return (
    <div
      className={cn(
        ...
        <p className="mt-1 text-xs text-[var(--ether-on-surface-variant)]">
          {state?.lastError
            ...
            : state?.lastSucceededAt
              ? `Last success ${formatDateTimeLabel(state.lastSucceededAt)}`
              : "Waiting for the first tenant sync run"}
        </p>
      </div>
      <Badge
        className={cn(
          "w-fit rounded-full border-0 px-3 py-1 text-[10px] font-bold uppercase",
          danger
            ? "bg-[var(--ether-error)] text-white"
            : warning
              ? "bg-amber-200 text-amber-900"
              : "bg-white text-[var(--ether-primary)]"
        )}
      >
