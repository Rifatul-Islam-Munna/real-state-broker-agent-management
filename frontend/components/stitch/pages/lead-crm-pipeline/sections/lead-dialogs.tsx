          ...
          <div className="space-y-8">
            ...
            <section>
              ...
              <div className="grid gap-5 md:grid-cols-3">
                <label className="space-y-2"><span className={fieldLabelClassName}>Budget / Value <span className="font-normal">(optional)</span></span><Input autoComplete="off" className={inputClassName} inputMode="decimal" onChange={(e) => updateField("budget", e.target.value)} placeholder="e.g. $290,000" value={formValues.budget} /><FieldError error={errors.budget} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Credit Score <span className="font-normal">(optional)</span></span><Input autoComplete="off" className={inputClassName} inputMode="numeric" onChange={(e) => updateField("creditScore", e.target.value)} placeholder="e.g. 720" value={formValues.creditScore} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Combined Credit <span className="font-normal">(optional)</span></span><Input autoComplete="off" className={inputClassName} inputMode="numeric" onChange={(e) => updateField("combinedCreditScore", e.target.value)} placeholder="e.g. 740" value={formValues.combinedCreditScore} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Monthly Earning <span className="font-normal">(optional)</span></span><Input autoComplete="off" className={inputClassName} inputMode="decimal" onChange={(e) => updateField("monthlyEarning", e.target.value)} placeholder="e.g. $6,500" value={formValues.monthlyEarning} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Combined Monthly Earning <span className="font-normal">(optional)</span></span><Input autoComplete="off" className={inputClassName} inputMode="decimal" onChange={(e) => updateField("combinedMonthlyEarning", e.target.value)} placeholder="e.g. $11,000" value={formValues.combinedMonthlyEarning} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Source <span className="font-normal">(optional)</span></span><Select modal={false} onValueChange={(value) => updateField("source", !value || value === emptySelectValue ? "" : value)} value={formValues.source || emptySelectValue}><SelectTrigger className={formSelectClassName}><span>{selectedSourceLabel}</span></SelectTrigger><SelectContent><SelectItem value={emptySelectValue}>Select source</SelectItem>{sourceOptions.map((source) => <SelectItem key={source} value={source}>{source}</SelectItem>)}</SelectContent></Select><FieldError error={errors.source} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Interest Type <span className="font-normal">(optional)</span></span><Select modal={false} onValueChange={(value) => updateField("interest", !value || value === emptySelectValue ? "" : value)} value={formValues.interest || emptySelectValue}><SelectTrigger className={formSelectClassName}><span>{selectedInterestLabel}</span></SelectTrigger><SelectContent><SelectItem value={emptySelectValue}>Select interest</SelectItem>{interestOptions.map((interest) => <SelectItem key={interest} value={interest}>{interest}</SelectItem>)}</SelectContent></Select><FieldError error={errors.interest} /></label>
                <label className="space-y-2"><span className={fieldLabelClassName}>Timeline <span className="font-normal">(optional)</span></span><Select modal={false} onValueChange={(value) => updateField("timeline", !value || value === emptySelectValue ? "" : value)} value={formValues.timeline || emptySelectValue}><SelectTrigger className={formSelectClassName}><span>{selectedTimelineLabel}</span></SelectTrigger><SelectContent><SelectItem value={emptySelectValue}>Select timeline</SelectItem>{timelineOptions.map((timeline) => <SelectItem key={timeline} value={timeline}>{timeline}</SelectItem>)}</SelectContent></Select><FieldError error={errors.timeline} /></label>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="mt-4 grid gap-4"><label className="space-y-2"><span className={fieldLabelClassName}>Summary <span className="font-normal">(optional)</span></span><Textarea className="min-h-24 rounded-lg border-[var(--ether-outline-variant)]" onChange={(e) => updateField("summary", e.target.value)} placeholder="Lead needs, qualification, and interest summary" value={formValues.summary} /><FieldError error={errors.summary} /></label><label className="space-y-2"><span className={fieldLabelClassName}>Internal Notes <span className="font-normal">(optional)</span></span><Textarea className="min-h-24 rounded-lg border-[var(--ether-outline-variant)]" onChange={(e) => updateField("notes", e.target.value)} placeholder="Private notes for your team" value={formValues.notes} /></label></div>
            </section>
