...
export function filterBotActivity(
  ...
  return items.filter((item) => {
    ...
    if (filters.channel !== "all" && item.channel !== filters.channel)
      return false;
    if (filters.status !== "all" && item.conversationStatus !== filters.status)
      return false;
...
export function buildTestChatbotInput(
  question: string,
  propertyId?: number | null,
) {
  return {
...
export function splitIndexedKnowledge<T extends { sourceType?: string }>(
  items: T[],
) {
...
export function composeQuestionExamples(
  mainQuestion: string,
...
export function normalizeTestHumanText(value: string) {
  ...
  const tokens =
    text.match(
      /[a-z]+|(?:\$|usd)?\d+(?:[.,]\d+)?(?:k|grand|thousand)?|[/@.+-]+/g,
    ) ?? [];
  let normalized = tokens
    .map((token) => normalizeTestToken(token))
    .join(" ")
    .replace(/\s+/g, " ")
    .trim();
  normalized = normalized
    .replace(/\bstill up\b/g, "still available")
    .replace(/\bstill open\b/g, "still available")
    .replace(/\bcan i swing by\b/g, "can i visit")
    .replace(/\bcan i pull up\b/g, "can i visit")
    .replace(/\bcan i go there\b/g, "can i visit")
    .replace(/\bcan we go there\b/g, "can we visit")
    .replace(/\bi want to go there\b/g, "i want to visit")
    .replace(/\bcan i take a peek\b/g, "can i see inside")
    .replace(/\bcan we take a peek\b/g, "can we see inside")
    .replace(/\bwhen can we take a look\b/g, "when can we view the property")
    .replace(/\bwhat is it going for\b/g, "monthly rent")
    .replace(/\bwhat are they asking\b/g, "monthly rent");
  return normalized;
}

export function testChatbotConversationReply(value: string) {
  const text = normalizeTestHumanText(value);
  if (!text || text.length > 80) return null;

  if (
    /^(?:hi|hello|hey|hiya|greetings|yo)(?: there| bot| chatbot| friend)?(?: how are you| how is it going)?$|^good (?:morning|afternoon|evening)(?: how are you)?$|^(?:how are you|how is it going|what is up|sup)$/.test(
      text,
    )
  ) {
  ...
  if (
    /^(?:thanks|thank you|thanks a lot|thank you very much|appreciate it)$/.test(
      text,
    )
  ) {
    return "You're welcome!";
  }
  if (/^(?:bye|goodbye|see you|talk to you later|talk later)$/.test(text)) {
    return "Goodbye! Come back anytime if you need help with the property.";
  }
  if (
    /^(?:who are you|what are you|are you a bot|what can you do)$/.test(text)
  ) {
...
function normalizeTestToken(token: string) {
  ...
  if (
    !/^[a-z]+$/.test(token) ||
    token.length < 4 ||
    TEST_CANONICAL.includes(token)
  )
    return token;
  const candidates = TEST_CANONICAL.filter(
    (candidate) =>
      candidate[0] === token[0] &&
      Math.abs(candidate.length - token.length) <= 1 &&
      oneEditApart(token, candidate),
  );
...
export function parseTestChatbotRole(value: string) {
  const text = normalizeTestHumanText(value);
  if (
    /\b(i am|we are|this is|working as).{0,12}\b(realtor|broker|real estate agent|leasing agent|rental agent|buyers? agent)\b/.test(
      text,
    )
  )
    return "REALTOR" as const;
  if (/\b(realtor|broker|agent)\s+(here|speaking)\b/.test(text))
    return "REALTOR" as const;
  if (
    /\b(represent|representing|working for|acting for|on behalf of).{0,20}\b(buyer|client|tenant|renter|owner|seller)\b/.test(
      text,
    )
  )
    return "REALTOR" as const;
  ...
  if (
    /\b(showing|viewing|touring).{0,20}\b(for|with).{0,12}\b(client|buyer|tenant|renter)\b/.test(
      text,
    )
  )
    return "REALTOR" as const;
  if (
    /^(?:yes |yeah |yep )?(?:(?:i am|we are) )?(?:a |an )?(realtor|broker|real estate agent|leasing agent|rental agent)$/.test(
      text,
    )
  )
    return "REALTOR" as const;
  ...
  if (
    /^(?:yes |yeah |yep )?(?:(?:i am|we are) )?(?:a |an )?(tenant|renter|applicant|prospective tenant|prospective renter)$/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(for myself|for ourselves|for me|for us|just me|my family|our family|my partner|my spouse|my husband|my wife|my girlfriend|my boyfriend|personal use)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(i|we|my|our).{0,18}\b(family|household|wife|husband|spouse|partner|kids?|children).{0,28}\b(fit|stay|live|move|enough|space|room)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(family|household)\s+(?:of\s+)?\d{1,2}.{0,24}\b(fit|stay|live|move|enough|space|room)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(i|we).{0,8}\b(want|would like|plan|hope|need|looking|interested|trying).{0,35}\b(rent|lease|move|live|buy|purchase)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(i|we).{0,8}\b(need|want|looking for|trying to find).{0,24}\b(place|home|house|apartment|unit|condo)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
  if (
    /\b(rent|lease|move into|live in|buy|purchase).{0,30}\b(myself|ourselves|me|us|my family|my partner|my spouse|this property|this place|this home|this house|this apartment|this unit|this condo)\b/.test(
      text,
    )
  )
    return "LEAD" as const;
...
export function parseTestShowingIntent(value: string) {
  const text = normalizeTestHumanText(value);
  return Boolean(
    /\b(showing|viewing|tour|visit|walk through|walkthrough|open house)\b/.test(
      text,
    ) ||
    /\b(show me around|show me (?:the )?(?:place|property|home|house|apartment|condo|unit)|take a look|look around|see inside|get inside|check it out|see it in person|physically see|come see|meet there)\b/.test(
      text,
    ) ||
    /\b(schedule|book|arrange|set up|reserve|make|get).{0,24}\b(visit|appointment|showing|viewing|tour|time|slot|walk through)\b/.test(
      text,
    ) ||
    /\b(can|could|would|may)\s+(?:i|we).{0,18}\b(see|view|tour|visit|walk|check).{0,20}\b(it|inside|there|place|property|unit|apartment|condo|house|home)\b/.test(
      text,
    ) ||
    /\b(i|we).{0,16}\b(want|would love|would like|hope|need|trying).{0,18}\b(see|view|tour|visit|walk through|check out).{0,18}\b(it|this|there|inside|property|place|home|house|apartment|unit|condo)\b/.test(
      text,
    ) ||
    /\b(would love|would like|love to|want to|trying to).{0,12}\b(see|view|tour|visit|walk through|check out).{0,18}\b(it|this|there|inside|property|place|home|house|apartment|unit|condo)\b/.test(
      text,
    ) ||
    /\b(is it|would it be).{0,20}\b(possible|okay|ok).{0,20}\b(see|view|visit|tour|come by|come over)\b/.test(
      text,
    ) ||
    /\b(see|view|visit|check out|look at).{0,28}\b(place|property|home|house|apartment|condo|unit)\b/.test(
      text,
    ) ||
    /\b(come by|come over|stop by|drop by|swing by|pull up|stop in|see it in person|physically see|meet there)\b/.test(
      text,
    ) ||
    /\bwhen\b.{0,28}\b(showing|viewing|visit|tour|see|come by|come over)\b/.test(
      text,
    ) ||
    /\b(showing|viewing|visit|tour|slot|appointment)\b.{0,28}\b(possible|available|tomorrow|today|tonight|weekend|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/.test(
      text,
    ),
  );
}
...
export function parseTestCredit(value: string) {
  ...
  if (
    isRequirementQuestion(text, "creditScore") ||
    (testHasPropertyNumberContext(text) && !testHasCreditSelfContext(text))
  )
    return null;
  ...
  if (
    spoken !== null &&
    (testLooksLikeCreditAttempt(text) || isTerseTestCredit(text))
  )
    return spoken;
...
export function parseTestMonthlyIncome(value: string) {
  const text = normalizeTestHumanText(value).replace(/,/g, "");
  if (
    isRequirementQuestion(text, "monthlyEarning") ||
    (!testLooksLikeIncomeAttempt(text) && !isTerseTestIncome(text))
  )
    return null;
  const contextual =
    text.match(
      /\b(?:income|salary|earnings|pay|make|earn|get|receive|bring in|bring home|pull in|take home|gross|net|clear)\b.{0,60}/i,
    )?.[0] ?? text;
  const amount = parseTestHumanAmount(contextual);
  if (amount === null) return null;
  if (/\b(per year|a year|yearly|annual|annually)\b|\/\s*year\b/i.test(text))
    return Math.round(amount / 12);
  if (/\b(biweekly|every two weeks)\b/i.test(text))
    return Math.round((amount * 26) / 12);
  if (/\b(per week|a week|weekly)\b|\/\s*week\b/i.test(text))
    return Math.round((amount * 52) / 12);
  return amount;
}

export function testQualificationClarification(
  value: string,
  ...
  if (expected === "creditScore") {
    if (parseTestCredit(value) !== null || !testLooksLikeCreditAttempt(text))
      return null;
    const raw = text.match(/\b\d{3,5}\b/)?.[0] ?? null;
    if (raw) {
      const suggestions = testCreditTypoSuggestions(raw);
      if (suggestions.length)
        return `I got that you're giving me your credit score. ${raw} is outside the usual 300–850 range. Did you mean ${formatTestChoices(suggestions)}?`;
  ...
  if (
    parseTestMonthlyIncome(value) !== null ||
    !testLooksLikeIncomeAttempt(text)
  )
    return null;
...
function testLooksLikeCreditAttempt(text: string) {
  ...
  if (
    testParseCreditBand(text) !== null ||
    parseTestSpokenCredit(text) !== null
  )
...
function testCreditTypoSuggestions(raw: string) {
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (Number.isFinite(candidate) && candidate >= 300 && candidate <= 850)
...
function isRequirementQuestion(
  value: string,
  expected: "creditScore" | "monthlyEarning",
) {
  const text = normalizeTestHumanText(value);
  const topic =
    expected === "creditScore"
      ...
      : /\b(income|earnings?|salary|monthly income|monthly pay)\b/i;
  if (!topic.test(text)) return false;
  const selfReport =
    expected === "creditScore"
      ? testHasCreditSelfContext(text)
      : /\b(my|our)\s+(income|salary|earnings|pay|monthly|gross|net)|\b(i|we)\s+(make|earn|get|receive|bring in|bring home|pull in|take home|gross|net|clear)\b/i.test(
          text,
        );
  if (selfReport) return false;
  return /\?|\b(minimum|required|requirement|need|qualify|how much|what|at least|must have|must make)\b/i.test(
    text,
  );
}
...
function parseTestSpokenCredit(value: string) {
  ...
  if (full)
    return validTestCredit(
      base[full[1].toLowerCase()] +
        parseTestUnderHundred(full[2] ?? "", ones, teens, tens),
    );
  ...
  if (zeroStyle)
    return validTestCredit(
      base[zeroStyle[1].toLowerCase()] + ones[zeroStyle[2].toLowerCase()],
    );
  ...
  return compact
    ? validTestCredit(
        base[compact[1].toLowerCase()] +
          parseTestUnderHundred(compact[2], ones, teens, tens),
      )
    : null;
}

function parseTestUnderHundred(
  value: string,
...
function parseTestHumanAmount(value: string) {
  ...
  if (wordAmount)
    return (
      words[wordAmount[1].toLowerCase()] * 1000 + (wordAmount[2] ? 500 : 0)
    );
...
function isTerseTestIncome(value: string) {
  const text = normalizeTestHumanText(value);
  const selfIncome =
    /\b(my|our)\s+(income|salary|earnings|pay|monthly|gross|net|take home)|\b(i|we)\s+(make|earn|get|receive|bring in|bring home|pull in|take home|gross|net|clear)\b/i.test(
      text,
    );
  if (
    /\b(rent|deposit|fee|hoa|association|price|cost|application|security deposit)\b/i.test(
...
export function propertyQualification(payload?: Record<string, unknown>) {
  ...
  const creditMatch = description.match(
    /\bcredit\s*score\b[^\d]{0,20}(\d{3})\b/i,
  );
  const incomeMatch = description.match(
    /\$\s*([\d,]+(?:\.\d+)?)\s*(?:monthly\s+income|per\s+month|\/\s*month)/i,
  );
  const minimumCreditScore =
    Number(
      String(creditRaw ?? creditMatch?.[1] ?? "").replace(/[^\d.]/g, ""),
    ) || null;
  const minimumMonthlyIncome =
    Number(
      String(incomeRaw ?? incomeMatch?.[1] ?? "").replace(/[^\d.]/g, ""),
    ) || null;
...
export function qualificationMatchesProperty(
  ...
  return (
    (!requirements.minimumCreditScore ||
      creditScore >= requirements.minimumCreditScore) &&
    (!requirements.minimumMonthlyIncome ||
      monthlyIncome >= requirements.minimumMonthlyIncome)
  );
}
