...
import { Building2, Clock3, Mail, MapPin, Phone, TriangleAlert } from "lucide-react"

import {
  TenantShowingRequestForm,
  type TenantPublicShowingRequest,
} from "@/components/tenant-showing-request-form"
...
const apiBase = (process.env.NEXT_PUBLIC_API_BASE_URL ?? process.env.BASE_URL ?? "http://localhost:4000/api").replace(/\/$/, "")
...
async function getShowingRequest(host: string, token: string) {
  try {
    const response = await fetch(`${apiBase}/tenant-public/showing-requests/${encodeURIComponent(token)}`, {
      cache: "no-store",
      headers: { "x-tenant-host": host },
    })
    if (response.ok) return { request: (await response.json()) as TenantPublicShowingRequest, error: null, status: response.status }
    const payload = await response.json().catch(() => ({}))
    const message = Array.isArray(payload.message) ? payload.message[0] : payload.message
    return { request: null, error: message || "This showing request is unavailable.", status: response.status }
  } catch {
    return { request: null, error: "The showing request service is temporarily unavailable.", status: 503 }
  }
}

function ShowingRequestUnavailable({ message, expired }: { message: string; expired: boolean }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f3f0e8] p-5 sm:p-8">
      <section className="w-full max-w-xl rounded-[30px] border border-white/70 bg-white p-7 text-center shadow-[0_30px_100px_rgba(48,50,46,0.14)] sm:p-10">
        <span className="mx-auto flex size-16 items-center justify-center rounded-full bg-amber-50 text-amber-800">{expired ? <Clock3 className="size-8" /> : <TriangleAlert className="size-8" />}</span>
        <p className="mt-6 text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Showing request</p>
        <h1 className="mt-3 text-3xl font-bold tracking-[-0.04em] text-slate-950">{expired ? "This secure link has expired." : "This form is unavailable."}</h1>
...
export default async function TenantSitePage({ params }: TenantSitePageProps) {
  ...
  if (path[0] === "showing-request") {
    const token = path[1]
    if (!token) notFound()
    const result = await getShowingRequest(host, token)
    if (result.status === 404) notFound()
    if (!result.request) return <ShowingRequestUnavailable expired={result.status === 410} message={result.error ?? "This request is unavailable."} />
    return <TenantShowingRequestForm initialRequest={result.request} token={token} />
  }

  const site = await getTenantSite(host)
  if (!site) notFound()

  const primary = site.branding.primaryColor || "#0d766e"
    ...
    <>
      ...
      <main className="min-h-screen bg-white text-slate-950">
        <header className="border-b border-slate-200 bg-white">
          <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-10">
            <div className="flex items-center gap-3">
              {site.branding.logoUrl ? <Image alt={site.tenant.businessName} className="h-10 w-auto" height={40} src={site.branding.logoUrl} unoptimized width={160} /> : <span className="flex size-10 items-center justify-center rounded-xl text-white" style={{ backgroundColor: primary }}><Building2 className="size-5" /></span>}
        ...
        <section className="bg-slate-950 px-5 py-20 text-white sm:px-8 sm:py-24 lg:px-10">
          <div className="mx-auto max-w-7xl"><p className="text-xs font-bold uppercase tracking-[0.16em]" style={{ color: primary }}>Welcome</p><h1 className="mt-4 max-w-4xl text-4xl font-bold tracking-[-0.045em] sm:text-5xl lg:text-6xl">{headline}</h1><p className="mt-6 max-w-2xl text-base leading-8 text-white/65 sm:text-lg">{description}</p></div>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10">
          <div><p className="text-xs font-bold uppercase tracking-[0.14em]" style={{ color: primary }}>Listings</p><h2 className="mt-2 text-3xl font-bold">Featured properties</h2></div>
