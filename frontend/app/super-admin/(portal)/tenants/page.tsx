import { createTenantAction, extendTenantSubscriptionAction, getPlans, getPlatformDomain, getTenants, setTenantBlockedAction } from "@/lib/super-admin-actions"
...
export default async function TenantsPage() {
  const [tenants, plans, domain] = await Promise.all([getTenants(), getPlans(), getPlatformDomain()]) as [Tenant[], Plan[], { primaryDomain: string }]
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <form action={createTenantAction} className="grid gap-4 rounded-2xl border bg-white p-6 shadow-sm md:grid-cols-2 xl:grid-cols-4">
      ...
      <input className={field} minLength={8} name="password" placeholder="Owner login password" required type="password" />
      <p className="text-xs leading-5 text-[#646273] md:col-span-2 xl:col-span-4">This becomes the tenant owner&apos;s login password and is stored only as a secure hash. If Preferred subdomain is blank, a unique subdomain is generated from the business name under {domain.primaryDomain}.</p>
    ...
    <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      {tenants.length === 0 ? <tr><td className="px-5 py-10 text-center text-[#646273]" colSpan={5}>No tenants have been provisioned yet.</td></tr> : tenants.map(t => <tr className="border-t align-top" key={t.id}>
        <td className="px-5 py-4"><p className="font-semibold">{t.businessName}</p><p className="text-xs text-[#646273]">{t.slug}</p><p className="mt-1 text-xs font-medium text-[#4343d5]">{t.subdomain}.{domain.primaryDomain}</p></td>
