...
export class PropertyOperationsPublicService {
  constructor(
    @InjectRepository(PropertyOperationsPublicAccess) private readonly accessRepository: Repository<PropertyOperationsPublicAccess>,
    @InjectRepository(PropertyOperationsSubmission) private readonly submissionRepository: Repository<PropertyOperationsSubmission>,
    @InjectRepository(PropertyOperationsRecord) private readonly recordRepository: Repository<PropertyOperationsRecord>,
  ...
  async list(propertyId?: number) {
    ...
    const accesses = await this.accessRepository.find({ where: workspace ? { workspaceId: workspace.id } : {}, order: { createdAt: 'DESC' } });
    const workspaces = await Promise.all([...new Set(accesses.map((item) => item.workspaceId))].map((id) => this.operationsService.workspaceById(id)));
    ...
    return accesses.map((access) => this.accessDto(
      access,
      workspaces.find((item) => item.id === access.workspaceId),
      submissions.filter((item) => item.accessId === access.id).length,
    ));
  }
  ...
  async create(input: any) {
    ...
    if (!Number.isFinite(expiresAt.getTime()) || expiresAt.getTime() <= Date.now()) throw new BadRequestException('Public request expiry must be in the future.');
    ...
    const linkedAmount = linkedRecord && ['billing', 'finance'].includes(linkedRecord.moduleKey) ? Number(linkedRecord.amount ?? 0) : 0;
    ...
    await this.operationsService.logActivity('External access created', `${access.title} link created.`, workspace.id, linkedRecord?.id, {
  ...
  async revoke(id: number) {
    ...
    await this.operationsService.logActivity('External access revoked', `${access.title} link revoked.`, access.workspaceId, access.recordId ?? undefined, {
  ...
  async submissions(accessId: number) {
    ...
    return items.map((item) => ({
      id: String(item.id), responderName: item.responderName, responderEmail: item.responderEmail,
      responderPhone: item.responderPhone, notes: item.notes, response: item.response,
      attachmentUrls: item.attachmentUrls, submittedAt: item.createdAt.toISOString(), createdAt: item.createdAt.toISOString(),
    }));
  }
  ...
  async getRequest(token: string) {
    ...
    return {
      ...
      currency: settings.currency, welcomeMessage: settings.welcomeMessage, termsText: settings.termsText,
      requireName: settings.requireName, requireEmail: settings.requireEmail, requirePhone: settings.requirePhone,
      allowFileUploads: access.allowFileUploads, showPropertyAddress: settings.showPropertyAddress,
      propertyTitle: String(snapshot.title ?? `Property ${workspace.propertyId}`), propertyLocation: String(snapshot.location ?? ''),
      moduleKey: access.moduleKey, title: access.title, instructions: access.instructions, recipientLabel: access.recipientLabel,
      formSchemaJson: JSON.stringify(access.formSchema ?? []), expiresAt: access.expiresAt.toISOString(),
      remainingUses: Math.max(0, access.maxUses - access.useCount), requestStatus: access.status,
      ...
      paymentVerified: access.paymentVerified || record?.status === 'Paid', linkedRecordStatus: record?.status ?? null,
    };
  }
  ...
  async submit(token: string, input: any) {
    ...
    const submission = await this.submissionRepository.save(this.submissionRepository.create({
      accessId: access.id, responderName: String(input?.responderName ?? ''), responderEmail: String(input?.responderEmail ?? ''),
      responderPhone: String(input?.responderPhone ?? ''), notes: String(input?.notes ?? ''), response: responsePayload, attachmentUrls,
    }));
    access.useCount += 1;
    access.lastAccessedAt = new Date();
    ...
    if (access.recordId) {
      ...
      if (record) {
        ...
        record.payload = { ...(record.payload ?? {}), latestSubmissionId: submission.id, latestSubmissionAt: submission.createdAt.toISOString(), latestSubmission: responsePayload };
        await this.recordRepository.save(record);
      }
    }
    await this.operationsService.logActivity('External response submitted', `Response submitted for ${access.title}.`, access.workspaceId, access.recordId ?? undefined, {
  ...
  async getStatus(token: string) {
    ...
    return {
      ...
      history: history.map((item) => ({ id: String(item.id), responderName: item.responderName, notes: item.notes, response: item.response, attachmentUrls: item.attachmentUrls, createdAt: item.createdAt.toISOString() })),
    };
  }
  ...
  async updateStatus(token: string, input: any) {
    ...
    await this.recordRepository.save(record);
    await this.operationsService.logActivity('Public status updated', `${record.title} marked ${record.status}.`, access.workspaceId, record.id, {
  ...
  async createCheckout(token: string, successUrl: string, cancelUrl: string) {
    ...
    const checkout = await this.integrations.createCheckout({ amount, currency: access.paymentCurrency || 'USD', title: record.title, reference: String(record.id), successUrl, cancelUrl });
    access.paymentSessionId = checkout.sessionId;
    access.paymentTokenHash = this.hash(checkout.paymentToken);
    await this.accessRepository.save(access);
    await this.operationsService.logActivity('Checkout created', `Checkout created for ${record.title}.`, access.workspaceId, record.id, {
  ...
  async verifyCheckout(token: string, sessionId: string, paymentToken: string) {
    ...
    if (verification.paid) {
      ...
      await this.operationsService.logActivity('Payment verified', `${record.title} marked paid.`, access.workspaceId, record.id, {
  ...
  private accessDto(access: PropertyOperationsPublicAccess, workspace: any, submissionCount: number) {
    ...
    return {
      id: String(access.id), propertyId: workspace?.propertyId ?? 0, recordId: access.recordId == null ? null : String(access.recordId),
      moduleKey: access.moduleKey, title: access.title, instructions: access.instructions, recipientLabel: access.recipientLabel,
      recipientName: access.recipientName, recipientEmail: access.recipientEmail, recipientPhone: access.recipientPhone,
      status: access.status, formSchemaJson: JSON.stringify(access.formSchema ?? []), expiresAt: access.expiresAt.toISOString(),
      maxUses: access.maxUses, useCount: access.useCount, oneTime: access.oneTime, allowFileUploads: access.allowFileUploads,
      createdAt: access.createdAt.toISOString(), lastAccessedAt: access.lastAccessedAt?.toISOString() ?? null,
      completedAt: access.completedAt?.toISOString() ?? null, propertyTitle: String(snapshot.title ?? `Property ${workspace?.propertyId ?? ''}`),
      propertyLocation: String(snapshot.location ?? ''), publicUrl, accessToken: access.accessToken,
      qrDataUrl: `https://api.qrserver.com/v1/create-qr-code/?size=360x360&margin=2&data=${encodeURIComponent(publicUrl)}`,
      submissionCount, paymentAmount: access.paymentAmount == null ? null : Number(access.paymentAmount),
      paymentCurrency: access.paymentCurrency, paymentVerified: access.paymentVerified,
    };
  }
  ...
  private validateRequiredFields(schema: unknown[], response: Record<string, unknown>, attachments: string[]) {
    for (const raw of Array.isArray(schema) ? schema : []) {
      ...
      const missing = type === 'file' ? !attachments.length && !value : type === 'checkbox' ? value !== true : value == null || String(value).trim() === '';
      const label = String(field.label || key || 'Required field');
      if (missing) throw new BadRequestException(`${label} is required.`);
    }
  }
