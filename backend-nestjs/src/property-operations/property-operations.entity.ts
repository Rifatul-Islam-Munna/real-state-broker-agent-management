...
export class PropertyOperationsWorkspace {
  ...
  @Index({ unique: true }) @Column({ type: 'int' }) propertyId: number;
  @Column({ default: 'Active' }) status: string;
  @Column({ type: 'jsonb', default: () => "'{}'::jsonb" }) propertySnapshot: Record<string, unknown>;
...
export class PropertyOperationsModuleState {
  @PrimaryGeneratedColumn() id: number;
  @Column({ type: 'int' }) workspaceId: number;
  @Column({ length: 80 }) moduleKey: string;
  @Column({ length: 40, default: 'Not started' }) status: string;
...
export class PropertyOperationsRecord {
  ...
  @Column({ type: 'int' }) workspaceId: number;
  @Column({ length: 80 }) moduleKey: string;
  @Column({ length: 80, default: 'Item' }) recordType: string;
  @Column({ length: 240 }) title: string;
  @Column({ type: 'text', default: '' }) description: string;
  @Column({ length: 60, default: 'Open' }) status: string;
  @Column({ length: 40, default: 'Normal' }) priority: string;
  @Column({ length: 160, default: '' }) contactName: string;
  @Column({ length: 240, default: '' }) contactEmail: string;
  @Column({ length: 80, default: '' }) contactPhone: string;
  @Column({ length: 120, default: '' }) contactLabel: string;
...
export class PropertyOperationsPublicAccess {
  @PrimaryGeneratedColumn() id: number;
  @Column({ type: 'int' }) workspaceId: number;
  @Column({ type: 'int', nullable: true }) recordId: number | null;
  @Column({ length: 80 }) moduleKey: string;
  @Column({ length: 64 }) tokenHash: string;
  @Column({ length: 80 }) accessToken: string;
  @Column({ length: 240 }) title: string;
  @Column({ type: 'text', default: '' }) instructions: string;
  @Column({ length: 120, default: '' }) recipientLabel: string;
  @Column({ length: 160, default: '' }) recipientName: string;
  @Column({ length: 240, default: '' }) recipientEmail: string;
  @Column({ length: 80, default: '' }) recipientPhone: string;
  @Column({ length: 40, default: 'Active' }) status: string;
  @Column({ type: 'jsonb', default: () => "'[]'::jsonb" }) formSchema: unknown[];
  @Column({ type: 'timestamptz' }) expiresAt: Date;
  @Column({ type: 'int', default: 1 }) maxUses: number;
  @Column({ type: 'int', default: 0 }) useCount: number;
  @Column({ default: true }) oneTime: boolean;
  @Column({ default: true }) allowFileUploads: boolean;
...
export class PropertyOperationsSubmission {
  @PrimaryGeneratedColumn() id: number;
  @Column({ type: 'int' }) accessId: number;
  @Column({ length: 160, default: '' }) responderName: string;
  @Column({ length: 240, default: '' }) responderEmail: string;
  @Column({ length: 80, default: '' }) responderPhone: string;
  @Column({ type: 'text', default: '' }) notes: string;
  @Column({ type: 'jsonb', default: () => "'{}'::jsonb" }) response: Record<string, unknown>;
  @Column({ type: 'jsonb', default: () => "'[]'::jsonb" }) attachmentUrls: string[];
  @CreateDateColumn({ type: 'timestamptz' }) createdAt: Date;
}
...
export class PropertyOperationsPreferences {
  @PrimaryGeneratedColumn() id: number;
  @Column({ type: 'jsonb', default: () => "'{}'::jsonb" }) content: Record<string, unknown>;
  @UpdateDateColumn({ type: 'timestamptz' }) updatedAt: Date;
}
