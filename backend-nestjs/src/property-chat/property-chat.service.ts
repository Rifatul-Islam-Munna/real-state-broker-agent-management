...
export class PropertyChatService {
    ...
    if (hasMinimumLeadData) {
      ...
      if (!lead) {
        lead = this.leadRepo.create({
          ...
          summary,
          notes: summary ? [summary.slice(0, 600)] : [],
      ...
      await this.historyRepo.save(this.historyRepo.create({
        ...
        direction: 'Incoming',
        status: 'Received',
        title: `Property chat for ${property.title}`,
        summary: summary || 'Property chat received.',
        body: summary,
        createdBy: 'System',
      }));
    }
  ...
  private buildSummary(dto: any) {
    ...
    for (const answer of dto.answers ?? []) {
      if (answer.answerText) lines.push(`${answer.questionPrompt || 'Pre-question'}: ${answer.answerText}`);
    }
  ...
  private buildMessages(dto: any) {
    ...
    for (const answer of dto.answers ?? []) {
      ...
      if (answer.answerText || answer.attachmentUrl) {
        messages.push({
          message: answer.answerText || 'Shared a file.',
          senderRole: PropertyChatSenderRole.Visitor,
          attachmentUrl: answer.attachmentUrl ?? null,
          attachmentObjectName: answer.attachmentObjectName ?? null,
          createdAt: now,
        });
      }
    }
  ...
  private mapConversation(item: PropertyChatConversation) {
    return {
      ...
      autoQualified: item.autoQualified,
      status: item.status,
      leadId: item.leadId ?? null,
      createdAt: item.createdAt,
      updatedAt: item.updatedAt,
      messages: (item.messages ?? [])
        .sort((a, b) => Number(new Date(a.createdAt)) - Number(new Date(b.createdAt)) || a.id - b.id)
        .map((message) => ({
          id: message.id,
          senderRole: message.senderRole,
          message: message.message,
          attachmentUrl: message.attachmentUrl ?? null,
          attachmentObjectName: message.attachmentObjectName ?? null,
