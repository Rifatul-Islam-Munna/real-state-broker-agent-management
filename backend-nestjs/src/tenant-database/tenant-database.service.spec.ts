...
describe('TenantDatabaseService', () => {
  ...
  it('creates deterministic isolated database names and rejects unsafe identifiers', () => {
    ...
    expect(service.databaseNameForTenant(42, 'Blue Horizon Realty')).toBe('tenant_42_blue_horizon_realty');
    expect(() => service.getPool('tenant_42_blue_horizon')).not.toThrow();
    expect(() => service.getPool('tenant-42;DROP DATABASE master')).toThrow('Invalid tenant database name');
  });
  ...
  it('runs unapplied migrations inside a transaction and records the version', async () => {
    ...
    expect(calls.some((call) => call.sql.includes('CREATE TABLE IF NOT EXISTS tenant_property'))).toBe(true);
    expect(calls.some((call) => call.sql.startsWith('INSERT INTO tenant_schema_migration'))).toBe(true);
    expect(calls.at(-1)?.sql).toBe('COMMIT');
  });

  it('skips already-applied migrations and rolls back when a migration fails', async () => {
    const service = createService();
    ...
    expect(appliedPool.query.mock.calls.some(([sql]: [string]) => sql.startsWith('INSERT INTO tenant_schema_migration'))).toBe(false);
    ...
    await expect(service.runMigrations(failingPool)).rejects.toThrow('migration failed');
  ...
  it('reuses tenant connection pools and releases checked-out clients', async () => {
    ...
    const fakePool: any = { connect: jest.fn(async () => ({ query: jest.fn(), release })) };
  ...
  it('migrates every ready tenant independently and reports failures without stopping the fleet', async () => {
    ...
    jest.spyOn(service, 'getPool').mockImplementation((name: string) => ({ databaseName: name }) as any);
    jest.spyOn(service, 'runMigrations').mockImplementation(async (pool: any) => {
      if (pool.databaseName === 'tenant_2_beta') throw new Error('beta failed');
    });

    const result = await service.migrateAllTenantDatabases();
    expect(result).toEqual([
      { tenantId: 1, databaseName: 'tenant_1_alpha', success: true },
      { tenantId: 2, databaseName: 'tenant_2_beta', success: false, error: 'beta failed' },
    ]);
  });
  ...
  it('drops a newly created database when migration or seeding fails', async () => {
    ...
    jest.spyOn(service, 'runMigrations').mockRejectedValue(new Error('schema failed'));
    const drop = jest.spyOn(service, 'dropDatabase').mockResolvedValue(undefined);

    await expect(service.provisionDatabase('tenant_5_failure', {
      tenantId: 5,
      businessName: 'Failure Realty',
      owner: { id: 10, firstName: 'A', lastName: 'B', email: 'a@b.com', role: 'Agent' },
