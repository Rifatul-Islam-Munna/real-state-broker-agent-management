...
const RULES: IntentRule[] = [
  ...
  { id:'guest-parking', label:'guest parking', weight:5, pattern:/\b(guest|visitor|company|friend|family).{0,28}(parking|car spot|vehicle spot|spot)|\b(parking for (guests?|visitors?)|guest spot|visitor spot|where can my guest park|where can visitors park)\b/i },
  ...
  { id:'apply-url', label:'online rental application link', weight:5, pattern:/\b(where|how).{0,18}(apply|application)|\b(application link|apply online|application website|rental application website|apply url|online application|where is the application|send me the application|application form|start the application)\b|^(?:apply|application)\??$/i },
  ...
  { id:'income-generic', label:'income requirements', weight:4, pattern:/\b(income|earnings?|salary|make|pay|wages?).{0,36}(need|required|requirement|qualif|minimum|property|enough|must)|\b(need|required|minimum|requirement|must|enough).{0,36}(income|earnings?|salary|make|pay|wages?)\b|\b(income requirement|income req|how much gotta make|how much do i gotta make|how much do i need to make|do i make enough|salary needed)\b/i },
  ...
  { id:'credit', label:'minimum credit score', weight:5, pattern:/\b(credit score|credit requirement|required credit|credit rating|fico|fico minimum|what fico|score cutoff|how much credit|what credit|credit do i need|credit needed|minimum score|score needed|score required|what score|score to qualify|credit to qualify)\b/i },
  { id:'movein-cost', label:'move-in costs', weight:5, pattern:/\b(move\s*-?\s*in).{0,30}(cost|total|money|upfront|need|pay|cash)|\b(money|upfront|cash).{0,30}move\s*-?\s*in\b|\b(first month.{0,35}last month|how much down to move in|what do i need upfront|move in money|cash to get in|how much to get in)\b/i },
  ...
  { id:'pets', label:'pet policy', weight:5, pattern:/\b(pets?|dogs?|pupp(?:y|ies)|cats?|kittens?|animals?|pet friendly).{0,28}(allow|permit|policy|bring|live|okay|ok|accept|friendly)?|\b(pet policy|dogs allowed|cats allowed|fur baby)\b/i },
  { id:'parking', label:'parking', weight:5, pattern:/\b(parking|park|car space|car spot|vehicle spot|vehicle parking|suv parking|assigned spot|assigned parking|parking included|spot included|where.{0,16}(leave|put) my car|space for my car|where do i put my car)\b/i },
  { id:'rent', label:'monthly rent', weight:5, pattern:/\b(monthly rent|rent price|rent amount|asking rent|asking price|monthly payment|monthly housing cost|price per month|rent per month|costs? per month|how much.{0,22}(rent|per month|is it)|what do i pay every month|what is the rent|what is it going for|what are they asking|how much is this place|how much is the place|price a month)\b|^(?:price|rent)\??$/i },
  { id:'available', label:'availability date', weight:5, pattern:/\b(available from|availability date|availability|still available|is it available|available now|available yet|when.{0,16}available|move\s*-?\s*in availability|when.{0,16}move\s*-?\s*in|move\s*-?\s*in date|ready to move in|ready now|ready date|occupancy date|when.{0,15}get keys|when.{0,15}start lease|vacant date|still vacant|still for rent|already leased|has it been taken|is it taken)\b/i },
  ...
  { id:'occupancy-fit', label:'household size and occupancy suitability', weight:5, pattern:/\b(family|household|people|persons?|members?|adults?|kids?|children).{0,35}\b(fit|stay|live|occupy|enough|space|room)|\b(fit|enough|space|room).{0,35}\b(family|household|people|persons?|members?|adults?|kids?|children)|\b(family of|household of|there (?:are|is)|we (?:are|have|had)).{0,18}\b\d{1,2}\b.{0,24}\b(fit|stay|live|occupy)|\b(max(?:imum)? occupancy|occupancy limit|how many people|how many can live|how many can stay)\b/i },
  ...
  { id:'shopping', label:'nearby shopping malls', weight:5, pattern:/\b(shopping malls?|nearby shopping|shopping nearby|places? to shop|where can i shop|shop nearby|stores? nearby|mall access|malls? nearby|shopping center)\b/i },
  ...
  { id:'utilities', label:'included and tenant-paid utilities', weight:4, pattern:/\b(utilities?|bills?).{0,30}(included|covered|pay|tenant|rent|responsible)|\b(what utilities|which bills|what bills.{0,15}included|what is included|what comes with rent|what bills are on me|what do i cover|what does rent cover|included in rent)\b/i },
  ...
  { id:'lease', label:'minimum lease duration', weight:5, pattern:/\b(minimum lease|lease length|lease term|how long.{0,18}(rent|stay|lease)|shortest tenancy|minimum tenancy|month to month|how many months.{0,15}lease|how long do i have to stay|how long do i gotta stay|one year lease|1 year lease)\b/i },
  ...
  { id:'phone', label:'listing phone number', weight:5, pattern:/\b(listing|agent|contact).{0,18}(phone|number|call)|\b(phone number|who can i call|contact by phone|listing contact|who do i talk to|who can i reach|contact person)\b|\b\d{3}-\d{3}-\d{4}\b/i },
  { id:'email', label:'listing email', weight:5, pattern:/\b(listing|agent|contact).{0,18}(email|mail)|\b(email address|who can i email|contact by email|listing contact|who do i talk to|who can i reach|contact person)\b|\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i },
