...
export class TenantLegacyCompatibilityService {
  ...
  async createProperty(tenant: SaasTenant, body: any, actorUserId: number) {
    const row = await this.dashboard.createProperty(
      ...
      {
        title: body?.title,
        status: this.storagePropertyStatus(body?.status),
        payload: body ?? {},
      },
      actorUserId,
    );
    return this.databases.withTenantClient(
      ...
      async (client) => {
        ...
        return this.propertyItem(
          updated.rows[0] ?? { ...row, payload: nextPayload },
        );
      },
    );
  }
  ...
  async updateProperty(tenant: SaasTenant, body: any) {
        ...
        const result = await client.query(
         ...
         WHERE id = $1 RETURNING id, title, status, payload, created_at, updated_at`,
          [
            id,
            `${body?.title ?? payload.title ?? ''}`.trim(),
            this.storagePropertyStatus(body?.status),
    ...
    await this.dashboard.reindexPropertyKnowledge(tenant, id);
    return property;
  }
  ...
  private async linkMatchingUnlistedLeads(
    ...
    const matchingIds = result.rows
      .filter(
        (lead: any) =>
          this.propertyMatchKey(lead?.payload?.property) === propertyKey,
      )
  ...
  private async syncPropertyDocuments(
    ...
    const documentKeys = new Set(
      documents
        .map((item: any) =>
          `${item?.fileObjectName ?? item?.fileUrl ?? ''}`.trim(),
        )
        .filter(Boolean),
    );
    ...
    for (const row of managed.rows) {
      const key =
        `${row.payload?.fileObjectName ?? row.payload?.fileUrl ?? ''}`.trim();
    ...
    for (const document of documents) {
      ...
      const repositoryPayload = {
        ...
        description:
          current.description ??
          `Uploaded from ${propertyTitle || 'property'}.`,
        ...
        mimeType:
          `${document?.mimeType ?? current.mimeType ?? 'application/octet-stream'}`.trim(),
        propertyId,
        propertyTitle,
        requiresSignature: current.requiresSignature === true,
        sizeBytes: Math.max(
          0,
          Number(document?.sizeBytes ?? current.sizeBytes) || 0,
        ),
        tags: Array.isArray(current.tags) ? current.tags : ['Property'],
        title:
          `${document?.name ?? current.title ?? document?.fileName ?? 'Property document'}`.trim(),
  ...
  async deleteProperty(tenant: SaasTenant, body: any) {
    ...
    return this.databases.withTenantClient(
      ...
      async (client) => {
        const result = await client.query(
          'DELETE FROM tenant_property WHERE id = $1 RETURNING id',
          [id],
        );
  ...
  async listLeads(tenant: SaasTenant, query: any) {
    ...
    if (query?.id)
      return items.find((item: any) => item.id === Number(query.id)) ?? null;
    const filtered = this.filter(items, query);
    const sortBy = `${query?.sortBy ?? 'latestEmailAt'}`.trim().toLowerCase();
    const direction =
      `${query?.sortOrder ?? 'desc'}`.toLowerCase() === 'asc' ? 1 : -1;
    filtered.sort((left: any, right: any) => {
      if (sortBy === 'name') {
        const nameDelta = `${left?.name ?? ''}`.localeCompare(
          `${right?.name ?? ''}`,
          undefined,
          { sensitivity: 'base' },
        );
        ...
        const leftDate = new Date(
          left?.latestEmailAt ?? left?.lastActivityAt ?? left?.createdAt ?? 0,
        ).getTime();
  ...
  async updateLead(tenant: SaasTenant, body: any) {
    ...
    return this.databases.withTenantClient(
      ...
      async (client) => {
        const current = await client.query(
          'SELECT payload FROM tenant_lead WHERE id = $1',
          [id],
        );
        ...
        const result = await client.query(
         ...
         RETURNING id, full_name, email, phone, status, payload, created_at, updated_at`,
          [
            ...
            `${body?.email ?? ''}`.trim(),
            `${body?.phone ?? ''}`.trim(),
            `${body?.stage ?? body?.status ?? 'new'}`,
            JSON.stringify(payload),
          ],
        );
        if (
          `${body?.stage ?? body?.status ?? ''}`.toLowerCase() === 'canceled'
        ) {
          await client.query(
    ...
    return this.databases.withTenantClient(
      ...
      async (client) => {
        ...
        const items = result.rows.map((row: any) =>
          cleanResource === 'deals'
            ? this.dealItem(row)
            : this.genericItem(row),
        );
        if (query?.id)
          return (
            items.find((item: any) => item.id === Number(query.id)) ?? null
          );
        if (query?.slug)
          return (
            items.find(
              (item: any) => `${item.slug ?? ''}` === `${query.slug}`,
            ) ?? null
          );
        const filtered = this.filter(items, query);
        if (
  ...
  async genericCreate(tenant: SaasTenant, resource: string, body: any) {
    ...
    if (
      cleanResource === 'lead-collection-templates' &&
      body?.isActive !== false
    ) {
  ...
  private async syncRepositoryDocumentToProperty(
    ...
    const documentIndex = documents.findIndex(
      (item: any) =>
        (fileObjectName &&
          `${item?.fileObjectName ?? ''}` === fileObjectName) ||
  ...
  private async removeRepositoryDocumentFromProperty(
    ...
    const fileUrl = `${document?.fileUrl ?? ''}`.trim();
    const documents = (
      Array.isArray(payload.propertyDocuments) ? payload.propertyDocuments : []
    ).filter(
      (item: any) =>
        !(
          (fileObjectName &&
  ...
  async documentSummary(tenant: SaasTenant) {
    const page = await this.genericList(tenant, 'documents', {
      page: 1,
      pageSize: 500,
    });
    return {
  ...
  syncStatus() {
    return {
      ...
      syncing: false,
      lastSyncedAt: null,
      message: 'Configure this tenant integration in Settings.',
    };
  }
  ...
  leadCollectionFields() {
    return [
      ...
      },
      {
        ...
        dataType: 'text',
        writable: true,
        suggestedTransform: 'CreditScore',
        requiredByDefault: false,
      },
      {
        field: 'combinedCreditScore',
        label: 'Combined Credit Score',
        dataType: 'text',
        writable: true,
        suggestedTransform: 'CreditScore',
        requiredByDefault: false,
      ...
      {
        field: 'combinedMonthlyEarning',
        label: 'Combined Monthly Earning',
        dataType: 'text',
        writable: true,
        suggestedTransform: 'Number',
        requiredByDefault: false,
  ...
  async leadCollectionPrepareSource(tenant: SaasTenant, dto: any) {
    ...
    const linked = linkedPageConfig.enabled
      ? await loadConfiguredLinkedPage(input, linkedPageConfig).catch(
          () => null,
        )
      : null;
    return {
      ...source,
      sourceText: prepareLeadCollectionSource({
        htmlBody: source.sourceHtml,
        textBody: source.sourceText,
      }),
      ...
      linkedPageStatus: linkedPageConfig.enabled
        ? linked
          ? 'Loaded'
  ...
  async leadCollectionTest(tenant: SaasTenant, dto: any) {
    ...
    const mappings = buildLeadCollectionMappings(
      sourceText,
      Array.isArray(dto?.mappings) ? dto.mappings : [],
    );
    ...
    const template = {
      ...
      senderPatterns: senderPatterns.length
        ? senderPatterns
        : prepared.senderPatterns,
      mailboxTags: this.stringList(dto?.mailboxTags),
      subjectPattern:
        `${dto?.subjectPattern ?? prepared.subjectPattern ?? ''}`.trim(),
  ...
  async importGeneric(tenant: SaasTenant, resource: string, body: any) {
    const source = Array.isArray(body?.items)
      ...
      : Array.isArray(body?.rows)
        ...
        : [];
    const created: any[] = [];
    for (const item of source.slice(0, 1000))
      created.push(await this.genericCreate(tenant, resource, item));
    return {
  ...
  private async tenantLeadCollectionSource(tenant: SaasTenant, dto: any) {
    ...
    if (sourceMailInboxId > 0) {
      return this.databases.withTenantClient(
        ...
        async (client) => {
          ...
          if (!result.rowCount)
            throw new NotFoundException('Source inbox email not found.');
          ...
          return {
            ...
            sourceText: `${row.body ?? ''}`,
            sampleFromAddress: `${row.recipient_email ?? ''}`
              .trim()
              .toLowerCase(),
    ...
    const sourceType = ['PastedHtml', 'PastedText', 'UploadedHtml'].includes(
      requestedType,
    )
      ? requestedType
      : sourceHtml
  ...
  private propertyItem(row: any) {
    ...
    return {
      ...
      imageObjectNames: Array.isArray(payload.imageObjectNames)
        ? payload.imageObjectNames
        : [],
      keyAmenities: Array.isArray(payload.keyAmenities)
        ? payload.keyAmenities
        : [],
      documentRepositoryItemIds: Array.isArray(
        payload.documentRepositoryItemIds,
      )
        ? payload.documentRepositoryItemIds
        : [],
      neighborhoodInsights: Array.isArray(payload.neighborhoodInsights)
        ? payload.neighborhoodInsights
        : [],
      preQuestions: Array.isArray(payload.preQuestions)
        ? payload.preQuestions
        : [],
      sellPrediction: payload.sellPrediction || {
  ...
  private leadItem(row: any) {
    ...
    const propertyListingStatus = properties.length
      ? properties.some((property: any) => property?.status === 'published')
        ? 'Listed'
        : 'NotListed'
      : payload.propertyListingStatus === 'Listed'
        ? 'Listed'
        : 'NotListed';
    ...
    return {
      ...
      latestEmailAt: payload.latestEmailAt ?? null,
  ...
  private async mergeLeadPayload(
    tenant: SaasTenant,
    leadId: number,
    body: any,
  ) {
  ...
  private normalizeLeadPayload(input: any) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'combinedCreditScore')) {
      payload.combinedCreditScore = this.normalizeCreditScore(
        payload.combinedCreditScore,
      );
    }
    return payload;
  }
  ...
  private filter(items: any[], query: any) {
    ...
    const status = `${query?.status ?? query?.stage ?? ''}`
      .trim()
      .toLowerCase();
    const propertyListingStatus =
      `${query?.propertyListingStatus ?? query?.listingStatus ?? ''}`
        .trim()
        .toLowerCase();
    ...
    return items.filter((item) => {
      if (
        status &&
        `${item?.stage ?? item?.status ?? ''}`.toLowerCase() !== status
      )
        return false;
      if (
        propertyListingStatus &&
        `${item?.propertyListingStatus ?? ''}`.toLowerCase() !==
          propertyListingStatus
      )
        return false;
      if (dateParts) {
  ...
  private storagePropertyStatus(value: unknown) {
    ...
    if (['open', 'active', 'published', 'forsale', 'forrent'].includes(status))
      return 'published';
    if (
      [
        'closed',
